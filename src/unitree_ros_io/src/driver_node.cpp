...
public:
    Custom(const char* low_udp_addr, const char* high_udp_addr)
        : 
        // low_udp(LOWLEVEL),
        low_udp(LOWLEVEL, 8091, low_udp_addr, 8007),
        high_udp(8090, high_udp_addr, 8082, sizeof(HighCmd), sizeof(HighState)) // 192.168.123.161 if running on robot, 192.168.12.1 otherwise
    {
        ROS_INFO("low_udp: %s, high_udp: %s", low_udp_addr, high_udp_addr);
...
void cmdVelCallback(const geometry_msgs::Twist::ConstPtr &msg)
{
    if (!custom)
    {
        ROS_ERROR("custom object is null!");
        return;
    }
...
int main(int argc, char **argv)
{
    ...
    std::string low_addr, high_addr;
    nh.param<std::string>("low_addr", low_addr, "192.168.123.10");
    nh.param<std::string>("high_addr", high_addr, "192.168.12.1");
    custom = new Custom(low_addr.c_str(), high_addr.c_str());
